...
class WhatsappController extends Controller
{
    ...
    public function ChildTest(){
        ...
        $today_schools_lhr=ChildRosterUpdated::distinct('school_code')->where('lab','=','LHR')->where(function($query) use ($current_date) {
                $query->where(DB::raw("str_to_date(entry_start_time, '%d-%M-%y')"),"=","$current_date")->orWhere(DB::raw("str_to_date(entry_start_time, '%m/%d/%Y')"),"=","$current_date");
            })->count();
        ...
        $todaySum_LHR = TipTest::where('office','=','LHR')->where(DB::raw("date(Date_of_Received)"),"=","$current_date")->where('Received_Status','=','1')->sum('TotalCount');
